...
const curacelCard = function () {
  const playIcon = $('<i /> ').attr({
    class: 'fa-solid fa-expand',
    id: 'play-icon'
  })
  const innerDivv = $('<div />').attr({ class: 'icon-div' }).append($(playIcon))
    ...
    $('.card-content-div').each((ind, card) => {
      $(card).on('click', async function () {
        try {
          // Show news data ==============
          if ($(this).index() == 0)
            return createNewsDataWrapper('/scrapper/v1/savenews')
          if ($(this).index() == 1)
            return bitcoin('https://rest.coinapi.io/v1/exchanges')
