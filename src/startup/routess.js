...
const Authenticate_user = function (app) {
  app.post(
    ...
    asyncMiddleware(async (req, res) => {
      ...
      if (
        req.body.email == 'raybags@github.com' ||
        req.body.name == 'Raymond Baguma'
      )
        return res
          .status(500)
          .send('this is a demo account. use your email address please')

      if (user)
        return res.status(400).json({
          message: 'User already exists! Contact admin for your token'
        })
...
const Get_user = function (app) {
  app.get(
    '/scrapper/v1/user/:email',
    asyncMiddleware(async (req, res) => {
      let user = await User.findOne({
        email: req.params.email
      })
      if (!req.params.email)
        return res.status(500).send('provide an email address')
      if (!user)
  ...
  app.delete(
    ...
    asyncMiddleware(async (req, res) => {
      ...
      if (!requestBody)
        return res
          .status(401)
          .json('name, email and password required to delete account')

      if (requestBody.email == '' || requestBody.password == '')
        return res
          .status(400)
          .json("name, email and password can't be empty string")

      let deletedUser = await User.findOne({ email: requestBody.email })
      if (!deletedUser)
        return res
          .status(401)
          .json('these credentials have been deleted or do not exist')
      ...
      res.status(200).json({
        message: {
          account: req.body,
          details: `All done "${user.name}!!" Your account has been deleted.`
        }
      })
    })
  )
...
const DeleteCollection = function (app) {
  app.delete(
    `/scrapper/v1/sky-news/:collection`,
    asyncMiddleware(async (req, res) => {
      let requestBody = req.body
      if (requestBody.token == '')
        return res
          .status(401)
          .json('Empty string! A token is required for this operation')
      // get user from the database by their access token
      let user = await User.findOne({
      ...
      if (!user)
        return res
          .status(404)
          .json('you are not authorized to delete this resource')

      if (user.token == 'null')
        return res.status(500).json("Token can't be an empty string!")
      ...
      OperationsLogger.info(
        'Collection name: ' + collection + ', Operation: deleted!'
      )
...
const CreateAndSaveBreakingNews = function (app) {
  app.post(
    '/scrapper/v1/savenews',
    asyncMiddleware(async (req, res) => {
      let requestBody = req.body
      if (requestBody.token == '')
        return res
          .status(401)
          .json('Empty string! To Scrape this website you need a token')
      // get user from the database by their access token
      let user = await User.findOne({
        token: requestBody.token
      })
      if (!user)
        return res
          .status(404)
          .json('you are not authorized to use this resource')
      ...
      OperationsLogger.info(
        `Operation name: 'create data', statusode: ${res.statusCode}`
      )
...
const CreateAndSaveTravelNews = function (app) {
  ...
  app.post(
    ...
    asyncMiddleware(async (req, res) => {
      ...
      OperationsLogger.info(
        `Operation name: 'create travel news', statusode: ${res.statusCode}`
      )
